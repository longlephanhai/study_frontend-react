...
import { callApiCreateMultiplePartFour } from "../../../services/api";
...
const ImportQuestionsPartFour = (props: IProps) => {
  const { isModalOpenImport, setIsModalOpenImport } = props;
  const [dataExcel, setDataExcel] = useState<IPartFour[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            if (json && json.length > 0) setDataExcel(json as IPartFour[]);
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await callApiCreateMultiplePartFour(dataExcel);
...
export default ImportQuestionsPartFour;