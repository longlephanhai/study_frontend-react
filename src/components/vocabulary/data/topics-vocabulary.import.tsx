...
const ImportTopicsVocabulary = (props: IProps) => {

  const { isModalOpenImport, setIsModalOpenImport } = props;

  const [dataExcel, setDataExcel] = useState<ITopicVocabulary[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            if (json && json.length > 0) setDataExcel(json as ITopicVocabulary[]);
          }
        }
  ...
  const handleSubmit = async () => {
    try {
      const response = await callApiCreateMultipleTopicsVocabularies(dataExcel);
...
export default ImportTopicsVocabulary;