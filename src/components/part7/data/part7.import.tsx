...
import { callApiCreateMultiplePartSeven, callApiCreateMultiplePartSix } from "../../../services/api";
...
const ImportQuestionsPartSeven = (props: IProps) => {
  const { isModalOpenImport, setIsModalOpenImport } = props;
  const [dataExcel, setDataExcel] = useState<IPartSeven[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            const json = XLSX.utils.sheet_to_json(sheet, {
              header: ["options", "correctAnswer", "explanation", "category", "transcript", "questionContent", "reading"],
              range: 1 //skip header row
            });
            if (json && json.length > 0) setDataExcel(json as IPartSeven[]);
          }
        }
...
export default ImportQuestionsPartSeven;