...
import { callApiCreateMultiplePartTwo } from "../../../services/api";
...
const ImportQuestionsPartTwo = (props: IProps) => {
  const { isModalOpenImport, setIsModalOpenImport } = props;
  const [dataExcel, setDataExcel] = useState<IPartTwo[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            const json = XLSX.utils.sheet_to_json(sheet, {
              header: ["audioUrl", "options", "correctAnswer", "explanation", "category", "transcript"],
              range: 1 //skip header row
            });
            if (json && json.length > 0) setDataExcel(json as IPartTwo[]);
          }
        }
  ...
  const handleSubmit = async () => {
    try {
      const response = await callApiCreateMultiplePartTwo(dataExcel);
...
export default ImportQuestionsPartTwo;