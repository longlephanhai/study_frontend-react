...
interface IProps {
  data: IBackendRes<IModelPaginate<IWriting>>;
...
const TableWriting = (props: IProps) => {

  const { data, onChange, loading, setLoading } = props;
  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const columns: TableColumnsType<IWriting> = [
  ...
  const rowSelection: TableRowSelection<IWriting> = {
    selectedRowKeys,
