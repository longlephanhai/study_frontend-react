...
import { callApiCreateMultiplePartFive } from "../../../services/api";
...
const ImportQuestionsPartFive = (props: IProps) => {
  const { isModalOpenImport, setIsModalOpenImport } = props;
  const [dataExcel, setDataExcel] = useState<IPartFive[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            const json = XLSX.utils.sheet_to_json(sheet, {
              header: ["options", "correctAnswer", "explanation", "category", "questionContent"],
              range: 1 //skip header row
            });
            if (json && json.length > 0) setDataExcel(json as IPartFive[]);
          }
        }
  ...
  const handleSubmit = async () => {
    try {
      const response = await callApiCreateMultiplePartFive(dataExcel);
...
export default ImportQuestionsPartFive;