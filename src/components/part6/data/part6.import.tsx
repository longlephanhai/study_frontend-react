...
import { callApiCreateMultiplePartSix } from "../../../services/api";
...
const ImportQuestionsPartSix = (props: IProps) => {
  const { isModalOpenImport, setIsModalOpenImport } = props;
  const [dataExcel, setDataExcel] = useState<IPartSix[]>([]);
      ...
      if (status === 'done') {
        if (info.fileList && info.fileList.length > 0) {
          ...
          reader.onload = function (e) {
            ...
            const json = XLSX.utils.sheet_to_json(sheet, {
              header: ["options", "correctAnswer", "explanation", "category", "transcript", "questionContent"],
              range: 1 //skip header row
            });
            if (json && json.length > 0) setDataExcel(json as IPartSix[]);
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await callApiCreateMultiplePartSix(dataExcel);
...
export default ImportQuestionsPartSix;